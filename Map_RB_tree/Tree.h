...
private:
	...
	node<T, L>* find(int key); // поиск элемента по ключу
	...
	node<T, L>* max_key(node<T, L>*);
	node<T, L>* min_key(node<T, L>*);
	void delete_node(node<T, L>* active_node);
	void delete_node_2(node<T, L>* active_node);
	/*void double_black_node(node<T, L>* db_node);
	void double_black_leaf(node<T, L>* parent_of_leaf);*/
	...
	void double_black(node<T, L>* db_node);
	void double_black_2(node<T, L>*, bool is_fiction);
	void first_case(node<T, L>* db_node);
...
public:
	...
	void remove(int key); // removes element by requested key
	void insert(int key, L data); // inserts element with requested key and data
	void output_map();
	L find_value(int key);
};
...
template<class T, class L>
void map_rb_tree<T, L>::delete_node(node<T, L>* active_node)
{
	if (!(active_node->left_child) && !(active_node->right_child))
	{	// deleting node is a leaf (has no children)
		if (active_node->color)
		{	// deleting node is red and it can't be a root
		...
		{	// deleting node is black
			...
			{	// deleting node is a root and it is single node at tree
				delete active_node;
				this->root = nullptr;
			}
			else
			{	// deleting node is not a root
				// db
			}
		}
	...
	{	// deleting node is not a leaf
		if (active_node==this->root)
		{	// deleting node is a root
			if (active_node->left_child&&active_node->right_child)
			{	// root has 2 children
				...
				if (new_active_node->color)
				{	// new deleting node is red, we'll just remove it
					if (new_active_node->is_left_child())
						new_active_node->parent->left_child = new_active_node->left_child;
					else
						new_active_node->parent->right_child = new_active_node->left_child;
					if (new_active_node->left_child)
						new_active_node->left_child->parent = new_active_node->parent;
					delete new_active_node;
				}
				else
				{	// new deleting node is black, there will be a double black one
					// db
				}
				
			}
			else
			{	//root has 1 child, there are only 2 nodes in the tree
				if (active_node->left_child)
				{	// root has only left child
					this->root = active_node->left_child;
					active_node->left_child->parent = nullptr;
					active_node->left_child->color = false;
				}
				else
				{	// root has only right child
					this->root = active_node->right_child;
					active_node->right_child->parent = nullptr;
					active_node->right_child->color = false;
				}
				delete active_node;
			}
		}
		else
		{	// deleting node is not a leaf or root
			node<T, L>* new_active_node;
			if (active_node->left_child&&active_node->right_child)
			{	// deleting node has 2 children
				new_active_node = max_key(active_node->left_child);
				active_node->key = new_active_node->key;
				active_node->data = new_active_node->data;
				if (new_active_node->color)
				{	// new deleting node is red
					if (new_active_node->is_left_child())
						new_active_node->parent->left_child = new_active_node->left_child;
					else
						new_active_node->parent->right_child = new_active_node->left_child;
				...
				{	// new deleting node is black
					// db
				}
			}
			else
			{	// deleting node has only 1 child

				if (active_node->color)
				{	// deleting node is red. child is black
					if (active_node->is_left_child())
					{
						if (active_node->left_child)
						{
							active_node->parent->left_child = active_node->left_child;
							active_node->left_child->parent = active_node->parent;
						}
						else
						{
							active_node->parent->left_child = active_node->right_child;
							active_node->right_child->parent = active_node->parent;
						}
					}
					else
					{
						if (active_node->left_child)
						{
							active_node->parent->right_child = active_node->left_child;
							active_node->left_child->parent = active_node->parent;
						}
						else
						{
							active_node->parent->right_child = active_node->right_child;
							active_node->right_child->parent = active_node->parent;
						}
					}
					delete active_node;
				...
				{	// deleting node is black
					if (active_node->left_child)
					{
						if (active_node->left_child->color)
						{
							new_active_node = active_node->left_child;
							active_node->key = new_active_node->key;
							active_node->data = new_active_node->data;
					...
					{
						if (active_node->right_child->color)
						{
							new_active_node = active_node->right_child;
							active_node->key = new_active_node->key;
							active_node->data = new_active_node->data;
							delete_node(new_active_node);
						}
...
template<class T, class L>
inline void map_rb_tree<T, L>::delete_node_2(node<T, L>* active_node)
{
	if (!active_node->left_child&&!active_node->right_child)
	{	// deleting node is a leaf
		...
		{	// deleting node is black
			...
			{	// deleting node is black and not root
				...
				delete active_node;
				// db for fiction
				double_black_2(fiction_node, true);
			}
		}
	}
	else
...
void map_rb_tree<T, L>::remove(int key)
{
	node<T, L>* current = this->find(key);
	if (current)
	{
		this->delete_node_2(current);
...
node<T, L>* map_rb_tree<T, L>::find(int key)
{
	node<T, L>*current = root;
...
void map_rb_tree<T, L>:: insert(int key, L data)
{
...
template<class T, class L>
inline void map_rb_tree<T, L>::double_black(node<T, L>* db_node)
{
	node<T, L>* brother = db_node->brother();
	if ((db_node->parent->color)&&!(brother->color))
	{
...
template<class T, class L>
inline void map_rb_tree<T, L>::double_black_2(node<T, L>* db_node, bool is_fiction)
{
	node<T, L>* brother = db_node->brother();
	if (brother->color)
	{	// brother is red
...
template<class T, class L>
inline void map_rb_tree<T, L>::first_case(node<T, L>* db_node)
{
...
inline void map_rb_tree<T, L>::red_brother_case(node<T, L>* db_node, bool is_fiction)
{
	...
	double_black_2(db_node, is_fiction);
}
...
template<class T, class L>
L map_rb_tree<T, L>::find_value(int key)
{
